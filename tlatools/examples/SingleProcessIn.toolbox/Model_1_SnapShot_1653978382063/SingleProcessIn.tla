...
ReplicateSet == [i \in ReplicateProc1|-> IF i \in SubNodes THEN 4..5
                                 ELSE (**null**) 1..1]
definit == [self \in Nodes, i \in ReplicateSet |-> IF i \in SubNodes THEN i
                                                   ELSE 1]

Init == (* Global variables *)
        /\ x = defaultInitValue
        /\ pc = [self \in ProcSet, i \in ReplicateSet |-> CASE self \in Nodes /\ i \in SubNodes
                                                        -> <<"start","Z","Lbl_1">> /\ <<i>>
                                                        
                           [] self \in Nodes /\ i = 1 -> <<"SProc">> /\ <<1>>]
