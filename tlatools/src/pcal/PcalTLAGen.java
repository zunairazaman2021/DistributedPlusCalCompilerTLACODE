...
public class PcalTLAGen
{
    ...
    private Vector<String> pcgroup = new Vector<>(); //ZZ Code
...
//        tlacode.addElement("");
    ...
    private void GenReplicateSet(AST.Thread thread, String context, String astname) throws PcalTLAGenException
    {
        varReplicate = varReplicate + 1;
        StringBuffer buf = new StringBuffer(astname+thread.name+varReplicate+"replicate(self, i)  == ");
        String indentSpaces;
        addOneLineOfTLA("");
        ...
        //For Distributed PlusCal
            ...
            for (int i = 0; i < thread.body.size(); i++) {
              ...
              String disjunct = stmt.label + "(self, i)";
              if (   i != 0
                     && tlacodeNextLine.length() + 7 /* the 7 was obtained empirically */
                     + disjunct.length() > wrapColumn) {
                endCurrentLineOfTLA();
    ...
    private void GenProcess(AST.Process ast, String context) throws PcalTLAGenException
    {
        ...
        if(PcalParams.distpcalFlag) {
          for(AST.Thread thread : ast.threads) {
           ...
           //   System.out.println("Thread.body lbl: " +tStmt);
             //ZZ Code    
             //System.out.println("Print thread.replicate as: "+thread.name);
             if(thread.name == ""){
                GenLabeledStmt(tStmt, "thread");
              }else{
                GenLabeledStmt(tStmt, "Replicatedthread");
             }
              //End ZZ Code
             //Commenting for ZZ
             // GenLabeledStmt(tStmt, "thread");
...
//        tlacode.addElement("");
    ...
    private void GenLabeledStmt(AST.LabeledStmt ast, String context) throws PcalTLAGenException
    {
        ...
        System.out.println("GenLabel Stsmts" +ast.stmts);
        if (ParseAlgorithm.omitPC) {
        ...
        if (mp && (context.equals("procedure") || selfIsSelf)) { // self.equals("self")))
          ...
          // HC: is it consistent with all uses of selfIsSelf for distpcalFlag?
            ...
            //ZZ Code 
            if (context.equals("Replicatedthread")){
                sb.append("(self,i)");
              }
            else {
             sb.append("(self)");
        ...
        for (int i = 0 ; i < ast.stmts.size(); i++) {
          ...
          // System.out.println("Macro ast.Stmt"+ stmt);
           ...
           /*
            ...
            if (iFirst == iLast)
            {
                                ...
                                .equals("stack")) )
                {
                    ...
                    sb.append(sass.lhs.var);
                    sb.append(" EXCEPT "); //ZZ Code Except too
...
//            tlacode.addElement(sb.toString());
    ...
    private void GenWhen(AST.When ast, Changed c, String context, String prefix, int col) throws PcalTLAGenException
    {
        ...
        addOneTokenToTLA(prefix);
        System.out.println("Inside GenWhen, Context is: "+ context);
//        StringBuffer sb = new StringBuffer(prefix);
        TLAExpr exp = AddSubscriptsToExpr(ast.exp, SubExpr(Self(context)), c);
        addLeftParen(exp.getOrigin());
        addExprToTLA(exp);
...
//            tlacode.addElement(is.toString());
        ...
        if (! ParseAlgorithm.omitPC) {
          ...
          {  
              ...
              boolean useCase = st.processes.size() != 1;
              if (useCase) {
                //is.append("/\\ pc = [self \\in ProcSet |-> CASE "); //For ZZ Code
...
//          tlacode.addElement(sb.toString());
   ...
   //ZZ Code       addOneLineOfTLA(sb.toString());
            ...
            for (int i = 0; i < st.processes.size(); i++)
            {
                ...
                for (int t = 0; t < p.threads.size();t++){
                    ...
                    if (r.name !=""){
                         ...
                         sb.append((String) sv.elementAt(0));
                         sb.append(", i \\in ");
                         Vector svC = r.replicate.toStringVector();
                         col = sb.length();
                         sb.append((String) svC.elementAt(0));
                         for (int j = 1; j < svC.size(); j++)
                           {
                              vec.addElement(sb.toString());
                              sb = new StringBuffer(NSpaces(col));
                              sb.append((String) svC.elementAt(j));
                            }
                          sb.append(": ");
                          varReplicateNext = varReplicateNext + 1;
                          sb.append(p.name+r.name+varReplicateNext+"replicate");
                          sb.append("(self, i)");
        ...
        if (nextSSP.size() > 0)
            ...
            {   Vector v = (Vector) nextSSP.elementAt(i);
                ...
                for (int j = 0; j < v.size(); j++)
                {
                    ...
                    sb.append(line);
                    System.out.println("Next Line: "+sb.toString());
                    addOneLineOfTLA(sb.toString());
//                    tlacode.addElement(sb.toString());
...
//            depths[i] = parenDepth;
    ...
    private static TLAExpr SubBracket(TLAExpr sub)
    {
        if (sub != null)
        { 
            TLAExpr expr = sub.cloneAndNormalize();
...
/* -------------------------------------------------------------------------- */
    ...
    ////////////////////////////////////////////////////
    public void GenProcsNewRelease(){
        if(st.processes == null || st.processes.size() == 0)
          return;
        for(int i = 0; i < st.processes.size(); i++){
            StringBuffer ps = new StringBuffer();
            PcalSymTab.ProcessEntry proc = (PcalSymTab.ProcessEntry) st.processes.elementAt(i);
            String countp = "procs" + varProcsI++;
            ps.append("Gen"+countp+" == { <<p>>: p \\in ");
            addOneTokenToTLA(ps.toString());
            addExprToTLA(proc.id);
            addOneTokenToTLA(" }");
			endCurrentLineOfTLA();
			addOneLineOfTLA("");
            //Add Genprocs1 Genprocs2 etc, in pcgroup vector array
            pcgroup.add("Gen"+countp);
            GenSingleThreads(proc);
        }   
    }
    //////////////////////////////////////////////////////////////////////////////
    ////// Genprocs2Thread1 == { <<p,1>>: p \in ProcessesQ }                 /////
    ///// Genprocs2Thread2 == { <<p,2,q>>: p \in ProcessesQ, i \in SubNodes} /////
    /////////////////////////////////////////////////////////////////////////////
    public void GenSingleThreads(   PcalSymTab.ProcessEntry proc){
     //For each process: Generate single threads, skip first thread, count from next, add 1,2,3 take labels into account
     StringBuffer ps = new StringBuffer();
     if (st.processes == null || st.processes.size() == 0)
         return;
    //for(int i = 0; i < st.processes.size(); i++){
     //   PcalSymTab.ProcessEntry node = (PcalSymTab.ProcessEntry) st.processes.elementAt(i);   
     PcalSymTab.ProcessEntry node = proc;
        String genproc = "Genprocs" + varThreading++;
        for (int t= 1; t < node.threads.size(); t++){
            StringBuffer prs = new StringBuffer();
            prs.append(genproc+"Thread");
            StringBuffer pcstr = new StringBuffer(); //pcgroup string 
            pcstr.append(genproc+"Thread"+t);          //pcgroup string   
            String temppc = pcstr.toString();
            pcgroup.add(temppc);    
            AST.Thread r = node.threads.get(t);
            String threadname = r.name;
            TLAExpr threadtype;
            if (r.replicate.getOrigin() == null){
                threadtype = null;
            }else{
                threadtype = r.replicate;
            }
            if (threadtype != null){
                //replicated thread
                prs.append(t+" == { <<p,"+t+","+threadname+">>: p \\in ");
                addOneTokenToTLA(prs.toString());
                addExprToTLA(node.id);
                addOneTokenToTLA(", "+threadname+ " \\in ");
                addExprToTLA(threadtype);
                addOneTokenToTLA("}");
            }else{ //single Thread
                prs.append(t+" == { <<p,"+t+">>: p \\in ");
                addOneTokenToTLA(prs.toString());
                addExprToTLA(node.id);
                addOneTokenToTLA(" }");
            }
            endCurrentLineOfTLA();
            addOneLineOfTLA("");
           // System.out.println("SingleThreads "+ prs.toString());
        }
    //}
    }
    //End NewRelease
