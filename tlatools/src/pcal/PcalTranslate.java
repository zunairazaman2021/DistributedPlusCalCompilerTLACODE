...
public class PcalTranslate {
    ...
    public static AST.Assign MakeAssign(String id, TLAExpr exp,
        ...
        if(PcalParams.distpcalFlag) {
         ...
         TLAToken tok;
         if (threadIndex == 0){
         //   PcalSymTab.ProcessEntry pe = (PcalSymTab.ProcessEntry) st.processes.elementAt(threadIndex);   
           // AST.Thread thread = ast.threads.elementAt(threadIndex);
          
             tok = BuiltInToken(  ">>]"); //ZZ Code //Done
         }else if(Replicate != ""){
            tok = BuiltInToken( ", "+(threadIndex ) + ", i>>]"); //ZZ Code //Done
        }
         else{
             tok = BuiltInToken( ", "+(threadIndex ) + ">>]"); //ZZ Code //Done
         }
         expr.addToken(tok);
         sAss.lhs.sub = expr;
          }
        ...
        System.out.println("Update PC"+result.ass);
        return result ;
      }
        ...
        *********************************************************************/
        ...
        if(PcalParams.distpcalFlag) {
          if(threadIndex != NO_THREAD) {
           ...
           if(threadIndex == 0){
            toks.addElement(BuiltInToken( ">>]")); //ZZ Code //First    
           }else if(Replicate!=""){
            toks.addElement(BuiltInToken( ", "+String.valueOf(threadIndex) +", i"+ ">>]")); //ZZ Code //First
           }else{
           toks.addElement(BuiltInToken( ", "+String.valueOf(threadIndex) + ">>]")); //ZZ Code //First
           }
          }
        } // end Distributed PlusCal
        ...
        *********************************************************************/
        ...
        if(PcalParams.distpcalFlag) {
          ...
          for (AST.Thread thread : ast.threads) {
              //thread = ExplodeThread(thread, thread.name, thread.index);  //ZZ Code replicate
    ...
    ***********************************************************************/
    ...
    private static AST.Thread ExplodeThread(AST.Thread ast, Integer threadIndex) throws PcalTranslateException {
        ...
        *********************************************************************/
        ...
        newast.replicate = ast.replicate;
        newast.body = new Vector();
        System.out.println("ExplodeThread= Name : "+newast.name);
        System.out.println("ExplodeThread= ReplicateID : "+newast.replicate);
        AST.LabeledStmt thisLS = (ast.body.size() > 0)
            ? (AST.LabeledStmt) ast.body.elementAt(0) : null;
        AST.LabeledStmt nextLS = (ast.body.size() > 1)
            ? (AST.LabeledStmt) ast.body.elementAt(1) : null;
        while (i < ast.body.size()) {
            String next = (nextLS == null)
                ? "Done"
                : nextLS.label;
            newast.body.addAll(ExplodeLabeledStmt(thisLS, next, threadIndex, newast.name));
            //Come back here
            i = i + 1;
            thisLS = nextLS;
            nextLS = (ast.body.size() > i + 1)
